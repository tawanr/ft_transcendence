...
async def check_authorization_header(self):
    ...
    if bearer.lower() != "bearer":
        ...
        await self.ft_send_err(
            "disconnect", "Wrong type of authorization header: should be Bearer!!!"
        )
        return
...
async def check_authorization_payload(self):
    ...
    if self.user:
        ...
        await self.ft_send_err(
            "disconnect", "Invalid registration. Closing connection."
        )
...
async def check_jwt(self, token):
    ...
    if not token or not token.is_token_valid():
        ...
        await self.ft_send_err(
            "disconnect", "Unauthorized, invalid token. Closing connection."
        )
        raise self.CustomException("Unauthorized")
    else:
        print("Pass auth")
